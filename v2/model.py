...
class LstmRNN():
    ...
    def add_cell(self):
        ...
        self.all_output = [tf.gather(cell_output, int(cell_output.shape[0])-(i+1)) for i in range(self.num_steps)]
        self.cell_last = tf.gather(cell_output, int(cell_output.shape[0])-1, name='cell_last_output')

    def add_output_hidden(self):
        Ws_initializer = tf.truncated_normal_initializer(mean=0.0, stddev=1.0, dtype=tf.float32)
        bs_initializer = tf.constant_initializer(value=0.1, dtype=tf.float32)
        self.Ws_out = [tf.get_variable(shape=[self.lstm_size, self.output_size], initializer=Ws_initializer, name='Ws_'+str(i)) for i in range(self.num_steps)]
        self.bs_out = [tf.get_variable(shape=[self.output_size], initializer=bs_initializer, name='bs_'+str(i)) for i in range(self.num_steps)]
        self.l_out_y = [tf.matmul(self.all_output[i], self.Ws_out[i]) + self.bs_out[i] for i in range(self.num_steps)]
        self.features = tf.concat([self.l_out_y[i] for i in range(self.num_steps)], 1)
        self.Ws_out2 = tf.get_variable(shape=[self.output_size*self.num_steps, self.output_size], initializer=Ws_initializer, name='Ws_out')
        self.bs_out2 = tf.get_variable(shape=[self.output_size], initializer=bs_initializer, name='bs_out')
        self.l_out_y2 = tf.matmul(self.features, self.Ws_out2) + self.bs_out2
        #self.l_out_y = tf.reshape(l_out_y, shape=[-1, self.output_size], name='pred_value')
    def add_optimizer(self):
        self.close = tf.gather(tf.transpose(self.l_out_y2, [1,0]), 3)
        self.close_y = tf.gather(tf.transpose(self.Y,[1,0]), 3)
        self.cost = tf.reduce_mean(tf.square(self.l_out_y2 - self.Y)) + self.alpha*tf.reduce_mean(tf.square(self.close-self.close_y))
    ...
    def fit(self):
        seq, seq_y = preprocess.getSeq(self.num_steps)
    ...
    def predict(self, stockNo, date='20180518'):
        ...
        with open('./data/result_'+date+'.csv', 'w', newline='') as csvfile:
            ...
            for num, target in enumerate(seq):
                ...
                pred = self.sess.run([self.l_out_y2], feed_dict=feed_dict)[0].reshape([-1])
                if pred[3] > 0:
                    result.append(1)
                elif pred[3] == 0:
                    result.append(0)
